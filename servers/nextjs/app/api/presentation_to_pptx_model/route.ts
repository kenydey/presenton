...
async function getAllChildElementsAttributes({
  ...
  for (const childElementHandle of directChildElementHandles) {
    ...
    // If table/chart has been parsed, or element is a canvas/table screenshot target,
    // we don't need to go deeper (prevents turning inner nodes into extra shapes).
    if (
  ...
  const attributes = await element.evaluate((el: Element) => {
    ...
    function parseElementAttributes(el: Element) {
      ...
      if (tagName === "table") {
        ...
        const anyRowTrs = rowTrs.length > 0 ? rowTrs : (Array.from(tableEl.querySelectorAll("tr")) as HTMLTableRowElement[]);
        ...
        const rows = anyRowTrs
          .map((tr) => {
            ...
            const rowCells = cells.map((cell) => (cell.textContent || "").trim());
            if (columnsCount === 0) return rowCells;
            // Normalize to columns length.
            const padded = Array.from({ length: columnsCount }, (_, i) => rowCells[i] ?? "");
            return padded;
          })
      ...
      if (tagName === "svg" || tagName === "canvas") {
          ...
          if (target && target === el) {
            ...
            if (cfgStr) {
              try {
                ...
                const chartType = typeof cfg.chartType === "string" ? cfg.chartType : "line";
                ...
                const series = Array.isArray(cfg.series)
                  ? cfg.series
                      .map((s: any) => {
                        const name = typeof s?.name === "string" ? s.name : "Series";
                ...
                chartData = {
                  ...
                  categories,
                  series,
                  showLegend: typeof cfg.showLegend === "boolean" ? cfg.showLegend : undefined,
                  showLabels: typeof cfg.showLabels === "boolean" ? cfg.showLabels : undefined,
                  colors: Array.isArray(cfg.colors) ? cfg.colors.map((c: any) => String(c)) : undefined,
                };
              } catch {
                chartData = undefined;
              }
        ...
        if (!chartData && tagName === "svg") {
          ...
          const parsedType = hasPie ? "pie" : hasLine ? "line" : hasBar ? "bar" : "bar";
