...
class PptxPresentationCreator:
    ...
    def add_table(self, slide: Slide, table_model: PptxTableBoxModel):
        # `python-pptx` requires at least 1 row/col; we always keep row 0 as header.
        ...
        rows_count = max(body_row_count + 1, 1)  # +1 for header row
    ...
    def add_chart(self, slide: Slide, chart_model: PptxChartBoxModel):
        ...
        # `python-pptx` uses CategoryChartData for most of our supported charts.
        chart_data = CategoryChartData()
        chart_data.categories = categories

        for series_model in series_models:
            values = series_model.values or []
            # Normalize values length to categories length.
            if categories:
        ...
        if chart_model.colors:
            for idx, color in enumerate(chart_model.colors):
                ...
                try:
                    ...
                    # Some chart types use line/solid differently; best-effort.
                    try:
